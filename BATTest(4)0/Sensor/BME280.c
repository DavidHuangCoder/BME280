...
static uint8_t bmp280_read_register(struct i2c_m_sync_desc *const i2c, uint8_t reg_addr)
{
	uint8_t reg_data;
...
static void bmp280_write_register(struct i2c_m_sync_desc *const i2c, uint8_t reg_addr, uint8_t reg_data)
{
	uint8_t reg_data[] = {reg_addr,reg_data};
    struct io_descriptor *I2C_0_io;

	i2c_m_sync_get_io_descriptor(i2c, &I2C_0_io);
	i2c_m_sync_enable(i2c);
	i2c_m_sync_set_slaveaddr(i2c, 0x76, I2C_M_SEVEN);
	io_write(I2C_0_io, (uint8_t *)&reg_addr, 2);
}
